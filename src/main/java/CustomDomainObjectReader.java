...
/**
 * This is a customized org.springframework.data.rest.webmvc.json.DomainObjectReader
 * that fixes two issues with Money deserialization in the latest Spring Data Rest.
 * See XXX comments below.
 */
public class CustomDomainObjectReader extends DomainObjectReader {

	private final PersistentEntities entities;
	private final Associations associationLinks;

	public CustomDomainObjectReader(PersistentEntities entities, Associations associationLinks) {
	...
	<T> T doMerge(ObjectNode root, T target, ObjectMapper mapper) throws Exception {
		...
		for (Iterator<Entry<String, JsonNode>> i = root.fields(); i.hasNext();) {
			...
			if (!mappedProperties.isWritableProperty(fieldName)) {
				...
				//XXX: don't remove the JsonNode:
				// In case of Money, "amount" does not map to a (writable) property,
				// but is converted to Money.number by the MonetaryAmountDeserializer
				// https://github.com/spring-projects/spring-data-rest/issues/1881
				// https://github.com/spring-projects/spring-data-rest/issues/1911
				// https://github.com/spring-projects/spring-data-rest/issues/1928
//				i.remove();
				continue;
			}
			...
			rawValue.ifPresent(it -> {
				...
				if (child.isObject()) {
					...
					// XXX: don't treat Money as entity:
					//  Money is registered as EmbeddableType in the Hibernate metamodel
					//  (even though it isn't really an embeddable, but a value type with user type PersistentMoneyAmountAndCurrency)
					//  and here, isEntity just checks whether the type exists in the metamodel
